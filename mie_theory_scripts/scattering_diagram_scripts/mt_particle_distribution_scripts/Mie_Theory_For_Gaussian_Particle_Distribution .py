...
Log_Normal_Data = LogNormal(size_axis, mu=d, gsd=sigma_g) / np.linalg.norm(LogNormal(size_axis, mu=d, gsd=sigma_g))
...
ax.plot(size_axis, Log_Normal_Data, 'r-', label='Tami Bond LogNormal $\u03c3_{g}$=1.05' )
...
for counter, element in enumerate(range(len(Log_Normal_Data))):
    Weights_LogNormal.append([size_axis[counter], Log_Normal_Data[counter]])
...
S33_Weighted = []
S34_Weighted = []
incident_stokes_vector_1 = [1, 0, 1, 0]
...
U_vec_2darray = []
V_vec_2darray = []
for element0 in Weights_Gaussian:
    for element1 in mu:
        Matrix_Elements = PMS.MatrixElements(m_particle, wavelength, element0[0], element1, m_medium)
        ...
        resultant_stokes_vector = np.asarray(SAM.dot(np.transpose(incident_stokes_vector_1))).flatten()
...
S33_Weighted_2 = []
S34_Weighted_2 = []
incident_stokes_vector_2 = [1, 0, -1, 0]
...
V_vec2_2darray = []
for element0 in Weights_Gaussian:
    for counter, element1 in enumerate(mu):
        Matrix_Elements2 = PMS.MatrixElements(m_particle, wavelength, element0[0], element1, m_medium)
        ...
        resultant_stokes_vector2 = np.asarray(SAM2.dot(np.transpose(incident_stokes_vector_2))).flatten()
