...
# size axis range for gaussian data and plots
size_axis_gauss = np.arange(d - (sigma_s * 3), d + (sigma_s * 3), 1)
# size axis range for lognormal data and plots
b = 300
size_axis_lognorm = np.arange(d - b, d + b, 1)
Log_Normal_Data = LogNormal(size_axis_lognorm, mu=d, gsd=sigma_g) / np.linalg.norm(LogNormal(size_axis_lognorm, mu=d, gsd=sigma_g))
Gaussian_Data = Gaussian(size_axis_gauss, mu=d, sigma=sigma_s)
#print(sp.integrate.simps(Gaussian(size_axis, mu=d, sigma=sigma_g), size_axis, dx=1))
f, ax = plt.subplots(figsize=(6, 6))
ax.plot(size_axis_lognorm, Log_Normal_Data, 'r-', label='Tami Bond LogNormal $\u03c3_{g}$=' + str(sigma_g))
ax.plot(size_axis_gauss, Gaussian_Data, 'b-', label='Gaussian Dist. \u03bc=' + str(d) + ', \u03c3=' + str(sigma_s))
...
for counter, element in enumerate(range(len(Gaussian_Data))):
    Weights_Gaussian.append([size_axis_gauss[counter], Gaussian_Data[counter]])
