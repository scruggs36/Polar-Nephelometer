...
def Retrieval_v0(path, number_density, geom_stdev, diameter, sizes, wavelength, m_known,  added_noise_array, n_space, k_space, truncation_array):
    pf_2darray = []
    pf_2darray_R = []
    pf_arr = []
    theta_2darray = []
    ...
    for q in truncation_array:
        for s in added_noise_array:
            pf_2darray = []
            pf_2darray_R = []
            chi_square_space = []
            now = datetime.datetime.now()
            data_path = path + str(now.year) + '_' + str(now.month) + '_' + str(now.day) + '_' + str(now.hour) + '_' + str(now.minute) + '_' + str(now.second) + '_' + 'sigma_' + str(s) + '_trunc_' + str(q) + '_' + '/'
            try:
...
# truncation array
truncation = np.arange(20, 50, 5).astype(np.int)
...
Retrieval_v0(path=py_path, number_density=concentration, geom_stdev=sigma_g, diameter=d, sizes=size_array, wavelength=w_n, m_known=m,  added_noise_array=noise_sigma_array, n_space=cri_n_space, k_space=cri_k_space, truncation_array=truncation)
