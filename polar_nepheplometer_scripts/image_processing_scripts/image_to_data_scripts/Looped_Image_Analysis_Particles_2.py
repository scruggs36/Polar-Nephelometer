...
cv2.imwrite(Path_Save + '/BKG_Avg.PNG', im_BKGAvg)
cv2.imwrite(Path_Save + '/He_Avg.PNG', im_HeAvg)
cv2.imwrite(Path_Save + '/N2_Avg.PNG', im_N2Avg)
cv2.imwrite(Path_Save + '/Samp_Avg.PNG', im_SampAvg)
...
cv2.imwrite(Path_Save + '/He_Avg_Corrected.PNG', im_Helium_Corrected)
cv2.imwrite(Path_Save + '/N2_Avg_Corrected.PNG', im_N2Rayleigh)
cv2.imwrite(Path_Save + '/Samp_Avg_Corrected.PNG', im_Sample)
...
for counter, element in enumerate(cols_array):
    ...
    bound_transect = np.array(im_N2Avg[arr, element]).astype('int')
    if np.amax(bound_transect) < 65536:
        ...
        bound_transect_imsub = np.array(im_N2Rayleigh[arr, element]).astype('int') - int(round(np.mean(np.array(im_N2Rayleigh[arr, element])[0:10])))
...
for counter, element in enumerate(cols_array):
    arr = np.arange(top[counter], bot[counter], 1).astype(int)
    bound_transect = np.array(im_SampAvg[arr, element])
    if np.amax(bound_transect) < 65536:
        ...
        bound_transect_imsub = np.array(im_Sample[arr, element]).astype('int') - int(round(np.mean(np.array(im_Sample[arr, element])[0:10])))
...
for counter, element in enumerate(cols_array):
    arr = np.arange(top[counter], bot[counter], 1).astype(int)
    bound_transect = np.array(im_HeAvg[arr, element])
    if np.amax(bound_transect) < 65536:
        ...
        bound_transect_imsub = np.array(im_Helium_Corrected[arr, element]).astype('int') - int(round(np.mean(np.array(im_Helium_Corrected[arr, element])[0:10])))
...
for counter, element in enumerate(cols_array):
    arr = np.arange(top[counter], bot[counter], 1).astype(int)
    bound_transect = np.array(im_BKGAvg[arr, element])
    if np.amax(bound_transect) < 65536:
        ...
        bound_transect_imsub = np.array(im_BKGAvg[arr, element]).astype('int') - int(round(np.mean(np.array(im_BKGAvg[arr, element]))))
