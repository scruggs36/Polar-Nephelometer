...
Cal_Dir = '/home/austen/media/winshare/Groups/Smith_G/Austen/Projects/Nephelometry/Polar Nephelometer/Lens Calibration/10-24-2018/Calibration Images/png images'
...
def undistort_loop(img_path, save_path, DIM, K, D):
...
def undistort_image(img, DIM, K, D):
    h, w = img.shape[:2]
    map1, map2 = cv2.fisheye.initUndistortRectifyMap(K, D, np.eye(3), K, DIM, cv2.CV_16SC2)
    undistorted_img = cv2.remap(img, map1, map2, interpolation=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT)
...
undistort_loop(Cal_Dir, Save_Dir, DIM_0, K_0, D_0)
