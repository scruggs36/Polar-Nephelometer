...
Path_Samp_Dir = '/home/austen/media/winshare/Groups/Smith_G/Austen/Projects/Nephelometry/Polar Nephelometer/Data/02-13-2019/900nm_PSL/0.1s'
Path_N2_Dir = '/home/austen/media/winshare/Groups/Smith_G/Austen/Projects/Nephelometry/Polar Nephelometer/Data/02-13-2019/N2/0.1s'
...
Path_BKG_Dir = '/home/austen/media/winshare/Groups/Smith_G/Austen/Projects/Nephelometry/Polar Nephelometer/Data/01-23-2019/900/BKG/T1'
#Path_Save_Averaged = '/home/austen/Documents/Averaged'
#Path_Save_Undistorted = '/home/austen/Documents/Undistorted'
Path_Save = '/home/austen/Documents/02-13-2019_Analysis'
...
for element in cols_array:
    ...
    im_transect = im_Sample[arr, element]
...
im_f3a = ax3[0].imshow(im_SampAvg, cmap='gray')
im_f3b = ax3[1].imshow(im_Sample, cmap='gray')
...
for counter, element in enumerate(cols_array):
    ...
    bound_transect = im_SampAvg[arr, element]
    bound_transect_imsub = im_Sample[arr, element]
    bkg_pts = np.average(bound_transect[:10])
    bkg_pts_imsub = np.average(bound_transect[:10])
    ...
    background_Samp.append(bkg_pts)
    SD_Samp_imsub_corrected.append(np.asarray(transect_summed_imsub) - bkg_pts)
...
im_f4 = ax4[0, 0].imshow(im_Sample, cmap='gray')
# create an axes on the right side of ax. The width of cax will be 5%
# of ax and the padding between cax and ax will be fixed at 0.05 inch.
...
ax4[0, 1].imshow(im_Sample, cmap='gray')
...
ax4[1, 0].grid(True)
ax4[1, 1].plot(cols_array, SD_Samp, linestyle='-', color='red', label='SD: Sample')
ax4[1, 1].plot(cols_array, SD_Samp_imsub, linestyle='-', color='blue', label='SD: Sample - N2')
...
PhaseFunctionDF['Sample Intensity'] = SD_Samp
PhaseFunctionDF.to_csv(Path_Save + '/SD_Offline.txt')



f6, ax6 = plt.subplots(2, 2, figsize=(12, 7))
ax6[0, 0].plot(arr_ndarray_N2[50], bound_transect_ndarray_N2[50], 'b-', label='Raw: Int Sum=' + str(np.sum(bound_transect_ndarray_N2[50])) + ' Bkg Sum=' + str(np.average(bound_transect_ndarray_N2[50][:10])*len(bound_transect_ndarray_N2[50])))
ax6[0, 0].plot(arr_ndarray_N2[50], bound_transect_ndarray_N2_imsub[50], 'r-', label='Subtracted: Int Sum=' + str(np.sum(bound_transect_ndarray_N2_imsub[50])) + ' Bkg Sum=' + str(np.average(bound_transect_ndarray_N2_imsub[50][:10])*len(bound_transect_ndarray_N2_imsub[50])))
...
ax6[0, 1].plot(arr_ndarray_N2[200], bound_transect_ndarray_N2[200], 'b-', label='Raw: Int Sum=' + str(np.sum(bound_transect_ndarray_N2[200])) + ' Bkg Sum=' + str(np.average(bound_transect_ndarray_N2[200][:10])*len(bound_transect_ndarray_N2[200])))
ax6[0, 1].plot(arr_ndarray_N2[200], bound_transect_ndarray_N2_imsub[200], 'r-', label='Subtracted: Int Sum=' + str(np.sum(bound_transect_ndarray_N2_imsub[200])) + ' Bkg Sum=' + str(np.average(bound_transect_ndarray_N2_imsub[200][:10])*len(bound_transect_ndarray_N2_imsub[200])))
...
ax6[1, 0].plot(arr_ndarray_N2[300], bound_transect_ndarray_N2[300], 'b-', label='Raw: Int Sum=' + str(np.sum(bound_transect_ndarray_N2[300])) + ' Bkg Sum=' + str(np.average(bound_transect_ndarray_N2[300][:10])*len(bound_transect_ndarray_N2[300])))
ax6[1, 0].plot(arr_ndarray_N2[300], bound_transect_ndarray_N2_imsub[300], 'r-', label='Subtracted: Int Sum=' + str(np.sum(bound_transect_ndarray_N2_imsub[300])) + ' Bkg Sum=' + str(np.average(bound_transect_ndarray_N2_imsub[300][:10])*len(bound_transect_ndarray_N2_imsub[300])))
...
f7, ax7 = plt.subplots(figsize=(12, 7))
ax7.semilogy(cols_array, SD_Samp, linestyle='-', color='black', label='Raw 900nm PSL Scattering')
ax7.semilogy(cols_array, SD_N2, linestyle='-', color='green', label='N2 Scattering')
ax7.semilogy(cols_array, SD_He, linestyle='-', color='cyan', label='He Scattering')
ax7.semilogy(cols_array, SD_BKG, linestyle='-', color='red', label='Background Scattering')
